...
def open_playlist_name_popup(P_pistes_list):
    ...
    btn_submit = tkinter.Button(popup_name_btn_container, text="creer", command= lambda: name_is_valid(P_pistes_list, name.get(), popup_name))
...
def name_is_valid(P_pistes_list, P_name, P_popup):
    ...
    if len(P_name) > 0:
        create_playlist_m3u(P_pistes_list, P_name, P_popup)

#######################################################################################################################

def create_playlist_m3u(P_pistes_list, P_playlist_name, P_popup):
    ...
    try:
        with open(f"{P_playlist_name}.m3u", 'w', encoding='utf-8') as f:
            f.write("#EXTM3U8\n")
            for piste in P_pistes_list:
                f.write(f"#EXTINF:{piste.get_duration()},{piste.get_title()}, {piste.get_artist()}, {piste.get_genre()}\n")
                f.write(f"{piste.get_path()}\n")
